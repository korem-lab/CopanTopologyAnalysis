...
rule all:
    input:
       ...
       expand(join(config["edgeEmbeddingsDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_walks.edge_embeddings"),
              ...
              k=DIMENSIONS),
        # temp files
        expand(join(config["tempDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_walks.model"),
              graph_id=GRAPH_IDS,
              walk_length=WALK_LENGTHS,
              n_walks=N_WALKS,
              p=P_VALUES,
              q=Q_VALUES, 
              k=DIMENSIONS),
       expand(join(config["tempDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_walks.embeddings"),
              graph_id=GRAPH_IDS,
              walk_length=WALK_LENGTHS,
              n_walks=N_WALKS,
              p=P_VALUES,
              q=Q_VALUES, 
              k=DIMENSIONS),
       expand(join(config["tempDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_walks.edge_embeddings"),
              graph_id=GRAPH_IDS,
              walk_length=WALK_LENGTHS,
              n_walks=N_WALKS,
              p=P_VALUES,
       ...
       # cluster dict
        expand(join(config["clustersDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_clusters.json"),
              ...
              q=Q_VALUES,
              k=DIMENSIONS),
        expand(join(config["tempDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_clusters.json"),
              graph_id=GRAPH_IDS,
              walk_length=WALK_LENGTHS,
              n_walks=N_WALKS,
              p=P_VALUES,
       ...
       expand(join(config["distancesDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_pairwiseDistances.csv"),
              ...
              q=Q_VALUES,
              k=DIMENSIONS), 
        expand(join(config["tempDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_pairwiseDistances.csv"),
              graph_id=GRAPH_IDS,
              walk_length=WALK_LENGTHS,
              n_walks=N_WALKS,
              p=P_VALUES,
       ...
       expand(join(config["degreeDir"], "{graph_id}_node_degrees.csv"),
              graph_id=GRAPH_IDS), 
        expand(join(config["tempDir"], "{graph_id}_node_degrees.json"),
              graph_id=GRAPH_IDS), 
        expand(join(config["tempDir"], "{graph_id}_node_degrees.csv"),
              graph_id=GRAPH_IDS), 
       ...
       expand(join(config["distDegDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_distancesWithDegree.csv"),
              ...
              q=Q_VALUES,
              k=DIMENSIONS), 
        expand(join(config["tempDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_distancesWithDegree.csv"),
              graph_id=GRAPH_IDS,
              walk_length=WALK_LENGTHS,
              n_walks=N_WALKS,
              p=P_VALUES,
        ...
        expand(join(config["distDegDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_stats.csv"),
              ...
              q=Q_VALUES,
              k=DIMENSIONS)
        expand(join(config["tempDir"], "{graph_id}_{walk_length}Lw{n_walks}Nw{p}p{q}q{k}k_stats.csv"),
              graph_id=GRAPH_IDS,
              walk_length=WALK_LENGTHS,
              n_walks=N_WALKS,
              p=P_VALUES,
