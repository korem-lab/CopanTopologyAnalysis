...
library(tidyverse)
library(stringr)
blast_result <- read_table("workflow/out/blast/sample_1_0_02_blast_results_coverage98.out", guess_max = 40000)
...
query_ids <- read_table("workflow/out/copangraphs/sample_1_0_02_ids.txt")
```

# check if all nodes have blast hits!
```{r}
n_qseqs <- length(query_ids$qseqid)
n_seqs_with_blast_hits <- length(unique(blast_result$qseqid))
qseqs_with_blast_hits <- unique(blast_result$qseqid)

qseqs_no_hits <- setdiff(query_ids$qseqid, unique(blast_result$qseqid))
